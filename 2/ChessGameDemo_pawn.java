...
public class ChessGameDemo_pawn extends JPanel implements MouseListener, MouseMotionListener {
    ...
    public ChessGameDemo_pawn(String pi, int x, int y, int x_1, int y_1, int length){
...
/*
	...
	*/
			...
			for(int i=14; i>=0; i--) {
				for(int j=14; j>=0; j--) {
					if(data_values[i][j] <100 ) {
						if(count_p <length){
							data_values[i][j] = length;
							++count_p;
						}
						else data_values[i][j] = 111;
					}
				}
			}
	...
	private static void createAndShowGUI(String piece, int x, int y, int x1, int y1, int length) {
		...
		int xx = x;
		int yy = y;
        ChessGameDemo_pawn newContentPane = new ChessGameDemo_pawn(p, xx, yy, x1, y1, length);
