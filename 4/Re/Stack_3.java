...
public class Stack_3 extends JPanel implements MouseListener, MouseMotionListener{
	...
	public String getCUT(int bb_x_loc, int bb_y_loc, int wf_x_loc, int wf_y_loc) {
		// Calculating the GateWays for White Fighter ......
		int x_dis = 8 - bb_x_loc;
		int y_dis = 8 - bb_y_loc;
		int dist = y_dis > 0 ? y_dis : x_dis;
		List<String> gateway_points = new ArrayList<String>();
		for(int i=8; i<=dist; i--) {
			gateway_points.add(""+i+","+"8");
		}
		for(int i=7; i<=dist; i--) {
			gateway_points.add(""+i+","+"7");
		}
		for(int i=6; i<=dist; i--) {
			gateway_points.add(""+i+","+"6");
		}
		for(int i=5; i<=dist; i--) {
			gateway_points.add(""+i+","+"5");
		}

		// Finding the shortest trajectories end points from white fighter to the above gateway_points :
		List<String> gateway_final = new ArrayList<String> ();
		int counter = 0; int length = 8;
		while(counter>0) {
			for(String fs : gateway_points) {
				if(shortLength(fs, wf_x_loc, wf_y_loc) < length ) {
					length = shortLength(fs, wf_x_loc, wf_y_loc);
					gateway_final.add(fs); // Final points got here...!!!
				}
			}
			counter = 1;
			break;
	...
	public int shortLength(String fs, int x, int y) {
		return 2;
	}
