...
public class ChessGameDemo_king_adm extends JPanel implements MouseListener, MouseMotionListener {
    ...
    public ChessGameDemo_king_adm(String pi, int x, int y, int x_1, int y_1, int length){
...
// ###############   START  ################ ADMISSIBLE TRAJECTORIES GETTING DOCK ITEMS FROM DOCK ARRAY ######################################
	...
	Integer[][][] master_dock = new Integer[4][8][8]; 
	...
	for(int i=0; i<dock_x.size(); i++) { // DOCK NA BADHA J ELEMENTS MATE LOOP, EACH ITEM MATE AKHU SEARCH ---------------------------------
		...
		for(int e=0; e<8; e++) { // CREATING ANOTHER 8*8 TABLE WITH DISTANCES FOR EACH OF THE DOCK ITEMS -------------------
			for(int f=0; f<8; f++) {
				int x_dis = ((x_d - e) < 0 ? -(x_d-e) : (x_d-e));
				int y_dis = ((y_d - f) < 0 ? -(y_d-f) : (y_d-f));
				if(x_dis > y_dis) master_dock[i][e][f] = x_dis;
				else master_dock[i][e][f] = y_dis;
			}
		}
		/*for(int i=0; i<4; i++) {
			for(int j=0; j<8; j++) {
...
*/		
		...
		final JTable table = new JTable(grand_dock, columnNames);   // data_values, data_values_1  combine__x_y
	...
	private static void createAndShowGUI(String piece, int x, int y, int x1, int y1, int length) {
		...
		int xx = x;
		int yy = y;
        ChessGameDemo_king_adm newContentPane = new ChessGameDemo_king_adm(p, xx, yy, x1, y1, length);
		
		//Create and set up the window.
        JFrame frame = new JFrame("ChessGameDemo_king_adm");
    ...
    public static void main(String[] args) {
		...
		//ChessGameDemo_king_adm obj = new ChessGameDemo_king_adm(piece, x, y);
